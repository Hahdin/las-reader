...
export const processLine = (line, dispatch, getState) => {
  ...
  if (line.indexOf('~') >= 0) {
    ...
    if (getState().files['FILE_STATE'].headings > 0) {//finish off last heading
      rtn = '\r\t},\r'
      rtn += tab + '"' + line + '": {\r'
    }
    else {
  ...
  else if (getState().files['FILE_STATE'].IsAscii) {
    ...
    let ch = "DATA" + (getState().files['FILE_STATE'].dataCount + 1) + '":'
    rtn = tab + tab + qt + ch + '[' + line + ']'
    if (!getState().files['FILE_STATE'].IsFirst) {
